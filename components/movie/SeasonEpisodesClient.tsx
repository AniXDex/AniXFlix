      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="relative">
          <button 
            ...
            className="flex items-center gap-2 bg-[#141414] border border-white/10 hover:border-white/30 px-4 py-2.5 rounded-xl text-sm font-bold text-white transition-all w-full sm:w-auto"
          >
        ...
        {isLoading ? (
          ...
          episodes.map((ep: any) => (
            ...
            >
              ...
              <div className="p-4 md:p-6 md:pl-2 flex flex-col justify-center flex-1">
                <h3 className="text-lg font-bold text-white mb-1 group-hover:text-red-500 transition-colors">{ep.name}</h3>
                <span className="text-xs text-white/50 mb-3 font-semibold">{ep.runtime ? `${ep.runtime} min` : '45 min'}</span>
                <p className="text-sm text-white/70 line-clamp-2 md:line-clamp-3 leading-relaxed">{ep.overview || "No description available for this episode."}</p>
              </div>
            </Link>
          ))
        ) : (
