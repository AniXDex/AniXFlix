...
const ALL_SERVERS = [
  ...
  { name: "Cinezo", url: (id: string, isTv: boolean, s = 1, e = 1) => isTv ? `https://player.cinezo.live/embed/tv/${id}/${s}/${e}?autoplay=true&muted=false` : `https://player.cinezo.live/embed/movie/${id}?autoplay=true&muted=false` },
  // 2 - screenscape.me
  { name: "ScreenScape", url: (id: string, isTv: boolean, s = 1, e = 1) => isTv ? `https://screenscape.me/embed?tmdb=${id}&type=tv&s=${s}&e=${e}&autoplay=1&muted=false` : `https://screenscape.me/embed?tmdb=${id}&type=movie&autoplay=1&muted=false` },
  // 3 - cinemaos.tech
  { name: "CinemaOS", url: (id: string, isTv: boolean, s = 1, e = 1) => isTv ? `https://cinemaos.tech/player/${id}/${s}/${e}?autoplay=1&muted=false` : `https://cinemaos.tech/player/${id}?autoplay=1&muted=false` },
  // 4 - rozgarlelo.modiplay.xyz
  { name: "razor[Hindi]", url: (id: string, isTv: boolean, s = 1, e = 1) => isTv ? `https://rozgarlelo.modiplay.xyz/embed/tmdb/tv?id=${id}&s=${s}&e=${e}&autoplay=1&muted=false` : `https://rozgarlelo.modiplay.xyz/embed/tmdb/movie?id=${id}&autoplay=1&muted=false` },
  // 5 - embed.filmu.in
  { name: "FilmU", url: (id: string, isTv: boolean, s = 1, e = 1) => isTv ? `https://embed.filmu.in/embed/tv/${id}/${s}/${e}?autoplay=1&muted=false` : `https://embed.filmu.in/embed/movie/${id}?autoplay=1&muted=false` },
  // 6 - player.videasy.net
  { name: "Videasy", url: (id: string, isTv: boolean, s = 1, e = 1) => isTv ? `https://player.videasy.net/tv/${id}/${s}/${e}?autoplay=1&muted=false` : `https://player.videasy.net/movie/${id}?autoplay=1&muted=false` },
  // 7 - peachify.top
  { name: "Peachify", url: (id: string, isTv: boolean, s = 1, e = 1) => isTv ? `https://peachify.top/embed/tv/${id}/${s}/${e}?autoplay=1&muted=false` : `https://peachify.top/embed/movie/${id}?autoplay=1&muted=false` }
];
